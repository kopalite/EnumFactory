...
namespace EnumFactory
{
    ...
    public static class ServiceCollectionExtensions
    {
        public static void AddEnumFactory<TEnum, TServiceType>(this IServiceCollection services, Lifecycle lifecycle = Lifecycle.Scoped) where TEnum : struct, Enum where TServiceType : class
        {
            ...
            Factory<TEnum, TServiceType>.ServiceTypes = new ConcurrentDictionary<TEnum, Type>(serviceTypesMap);

            _ = lifecycle switch
            {
                Lifecycle.Scoped => services.AddScoped<IFactory<TEnum, TServiceType>, Factory<TEnum, TServiceType>>(),
                Lifecycle.Transient => services.AddTransient<IFactory<TEnum, TServiceType>, Factory<TEnum, TServiceType>>(),
                Lifecycle.Singleton => services.AddSingleton<IFactory<TEnum, TServiceType>, Factory<TEnum, TServiceType>>(),
                _ => throw new ArgumentOutOfRangeException(nameof(lifecycle))
            };
        }
